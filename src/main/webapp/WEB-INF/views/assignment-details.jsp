...
<%@ page import="com.mycompany.slateboard.pojo.Assignment"%>
<%@ page import="javax.servlet.http.HttpSession"%>
...
<head>
<meta charset="UTF-8">
<title>Slateboard - Assignment</title>
<link rel="stylesheet"
		...
		<div class="list-group">
			...
			<div class="card">
				<div class="card-header"><a href="/imagefolder/<%=assignment.getFilePath()%>"
					class="list-group-item"><%=assignment.getNotes()%></a> </div>
					<div class="class-footer"><span
					class="row"><label class="col-sm-3">Uploaded on: <%=uploadedOn.toString().split("T")[0]%>
						at <%=uploadedOn.toString().split("T")[1]%>
				</label> <label class="col-sm-3">Deadline:
						<%=deadline.toString().split("T")[0]%> at <%=deadline.toString().split("T")[1]%></label>
						<label class="col-sm-3"></label>
						<%
						if (role.equals("professor")) {
						%>
						<a href="delete-assignment.htm?assignmentId=<%=assignment.getId() %>" class="btn btn-link">Delete</button></span>
						<% } %>
			</div></div>
			<%
				}
